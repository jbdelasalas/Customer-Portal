 ...
 *
 * Issues a one-time password for a customer who cannot get in. Email delivery
 ...
 *    default across every customer;
 *  - leave the temporary password usable indefinitely — must_change_password
 *    forces the customer to set their own before doing anything;
...
export const POST = handler(
  async (request: NextRequest, { params }: { params: { id: string } }) => {
    ...
    await transaction(async (client) => {
      ...
      // Any reset links the customer already has are now void.
      await client.query(
