...
interface Issued {
  user: { email: string; fullName: string; customerCode: string | null };
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Reset a password for a customer who cannot sign in.
            </p>
          </div>
          <input
        ...
        {issued && (
          <div className="mt-6 rounded-lg border-2 border-green-300 bg-green-50 p-5">
            ...
            <p className="mt-1 text-sm text-green-800">
              {issued.user.email}
              {issued.user.customerCode ? ` · ${issued.user.customerCode}` : ''}
            </p>
            ...
            <p className="mt-4 text-sm text-green-900">
              Give this to the customer directly — by phone or in person. They will be asked to
        ...
        {/* Confirmation, because this signs the customer out everywhere. */}
        {confirming && (
          <div className="mt-6 rounded-lg border-2 border-amber-300 bg-amber-50 p-5">
            ...
            <p className="mt-2 text-sm text-amber-800">
              {confirming.email}
              {confirming.customer_code ? ` · ${confirming.customer_code}` : ''}
            </p>
                  ...
                  {users.map((u) => (
                    <tr key={u.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-600">
                        {u.customer_code ? (
                          <>
                            <div className="font-mono text-xs">{u.customer_code}</div>
                            <div className="text-xs text-slate-400">{u.customer_name}</div>
                          </>
                        ) : (
